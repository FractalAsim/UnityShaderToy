	...
	{	
		Pass
		{
			...
			void main()
			{
				...
				p = vec2(n*cos(p.x+0.1*_Time.y),p.y); 
				float y0 = p.y + 0.2*sin(p.x+_Time.y-1.0+0.5*cos(_Time.y));
				float y1 = p.y + 0.2*cos(p.x+_Time.y+1.0+0.5*sin(_Time.y));
				y0 *= y0;
				y1 *= y1;
