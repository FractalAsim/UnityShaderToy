Shader "GLSL/ FlappyBird" 
{
//http://glslsandbox.com/e#17444.0
...
// Flap your mouse up and down around the middle of the screen to make the bird fly.
	Properties
	{
		...
		backbuffer ("Back Buffer", 2D) = "white" {}
	}
	SubShader
	{	
		Pass
		{
			...
			cState loadState()
			{
				...
				cState state;
				state.validData =  texture2D(backbuffer,( vec2(0.4,0.0) / _ScreenParams.xy )).a;
				state.alive =      texture2D(backbuffer,( vec2(1.4,0.0) / _ScreenParams.xy )).a;
				state.deadTime =   texture2D(backbuffer,( vec2(2.4,0.0) / _ScreenParams.xy )).a;
				state.birdPosY =   texture2D(backbuffer,( vec2(3.4,0.0) / _ScreenParams.xy )).a * 800.0 - 250.0;
				state.birdVelY =   texture2D(backbuffer,( vec2(4.4,0.0) / _ScreenParams.xy )).a * 80.0 - 40.0;
				state.mouseRegion = texture2D(backbuffer,( vec2(5.4,0.0) / _ScreenParams.xy )).a;
				state.score =       texture2D(backbuffer,( vec2(6.4,0.0) / _ScreenParams.xy )).a * 255.0;
				state.hiscore =     texture2D(backbuffer,( vec2(7.4,0.0) / _ScreenParams.xy )).a * 255.0;
				state.xParity  =    texture2D(backbuffer,( vec2(8.4,0.0) / _ScreenParams.xy )).a;
			...
			void main(void)
			{
				...
				float s = 2000.0/_ScreenParams.x;
				vec2 p = max(1.6666667, s)*(gl_FragCoord.xy - _ScreenParams.xy * 0.5);
				
				float dx = _Time.y * 320.0;
				p.x += dx;
				...
				vec2 dp = ( gl_FragCoord.xy / _ScreenParams.xy );
